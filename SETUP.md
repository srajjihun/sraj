...
<https://srajjihun.github.io/sraj/ydp.html> — 즐겨찾기에 넣어 두세요.

**PC 폴더의 `ydp.html` 을 더블클릭해서 열면 안 됩니다.** 열리기는 하는데
공고가 하나도 없이 "로드 실패" 만 뜹니다. 페이지가 `data/*.json` 을
`fetch()` 로 읽는데, 브라우저는 `file://` 로 연 페이지의 이 요청을 보안상
차단하기 때문입니다. 파일은 멀쩡히 있는데 읽지 못하는 것입니다.

PC 가 하는 일은 **수집해서 GitHub 에 올리는 것**뿐입니다.
보는 것은 언제나 위 주소에서 하세요.
...
**무엇이 이상하든 `모집신청-설치.bat` 을 한 번 더 두 번 클릭해 보세요.**
명령 프롬프트는 필요 없습니다. 설치 파일은 `[1/4]` 에서 코드를 GitHub 의
최신 상태로 맞춥니다 - 클론이 다른 브랜치로 넘어갔거나 로컬과 원격이
갈라져 있어도 제자리로 돌려놓습니다. 그러고 나서 나머지를 다시 합니다.
직접 만드신 설정(`config\`)과 입찰 데이터(`data\g2b\`)는 그대로 남습니다.

그래도 안 되면 로그(`logs\collect.log`)를 보세요. 대부분 원인이 그대로
적혀 있습니다.
...
수집은 잘 되는데 **화면에 공고가 0건이고 "로드 실패"** 가 뜬다면, PC 의
`ydp.html` 을 직접 연 것입니다. 위 [보는 곳은 웹사이트입니다](#보는-곳은-웹사이트입니다) 를 보세요.
...
위 [직접 하려면](#직접-하려면) 의 `schtasks` 네 줄을 붙여넣으세요.
